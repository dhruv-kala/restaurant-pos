...
class _WaiterScreenState extends State<WaiterScreen> {
  ...
  final List<_OrderData> _orders = [
    _OrderData(
      ...
      items: ['2× Butter Paneer Masala', '3× Garlic Naan', '2× Dal Makhani', '6× Mango Lassi'],
    ),
    _OrderData(
      tableNo: 2,
      orderNo: '#141',
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            _buildMiniMetric(Icons.table_restaurant_rounded, _amber, _amberLight, 'My Tables', '8', '5 occupied · 3 free'),
                            const SizedBox(width: 16),
                            _buildMiniMetric(Icons.access_time_rounded, _red, _redLight, 'Pending', '3', '2 orders · 1 bill request', valueColor: _red),
                            const SizedBox(width: 16),
                            _buildMiniMetric(Icons.check_circle_outline_rounded, _green, _greenLight, 'Served today', '14', 'Across all tables'),
                            const SizedBox(width: 16),
                            _buildMiniMetric(Icons.timer_outlined, _amber, _amberLight, 'Avg. serve time', '18 min', 'Last 10 orders'),
                          ],
                        ),
                        ...
                        GridView.builder(
                          ...
                          itemBuilder: (_, i) => _TableTile(
                            ...
                            onTap: () => setState(() => _selectedTable = _tables[i].tableNo),
                          ),
                        ),
                        const SizedBox(height: 24),
                        ...
                        ..._orders.map((o) => Padding(
                              ...
                              child: _OrderCard(
                                order: o,
                                onMarkServed: () => _showSnack(context, 'Table ${o.tableNo} marked as served'),
                                onSendToCashier: () => _showSnack(context, 'Bill sent to cashier for Table ${o.tableNo}'),
                                onAddItems: () => _showSnack(context, 'Opening menu for Table ${o.tableNo}'),
                                onRequestBill: () => _showSnack(context, 'Bill requested for Table ${o.tableNo}'),
                              ),
                            )),
                      ],
                    ),
      ...
      child: Row(
        children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Waiter Panel', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: _textPrimary)),
            const Text('Section A · 8 tables assigned', style: TextStyle(fontSize: 12, color: _textSecondary)),
          ]),
          const Spacer(),
          if (pending > 0)
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
              decoration: BoxDecoration(color: _amberLight, borderRadius: BorderRadius.circular(20)),
              child: Row(
                children: [
                  const Icon(Icons.notifications_rounded, size: 14, color: _amber),
                  const SizedBox(width: 4),
                  Text('$pending pending', style: const TextStyle(fontSize: 11, color: _amber, fontWeight: FontWeight.w600)),
                ],
              ),
            ),
          ...
          CircleAvatar(
            ...
            child: const Text('PS', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _amber)),
          ),
        ],
      ),
    );
  ...
  Widget _buildMiniMetric(IconData icon, Color color, Color bg, String label, String value, String sub, {Color? valueColor}) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.all(18),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: _border, width: 0.5)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(value, style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: valueColor ?? _textPrimary)),
...
class _TableTile extends StatelessWidget {
  ...
  const _TableTile({required this.data, required this.selected, required this.onTap});
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: bg,
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: selected ? const Color(0xFF1A2035) : border, width: selected ? 2 : 1),
        ),
        padding: const EdgeInsets.all(12),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text('T${data.tableNo}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: textColor)),
            const SizedBox(height: 4),
            if (data.status != TableStatus.free)
              Row(
                ...
                children: [
                  ...
                  Text('${data.guests}', style: TextStyle(fontSize: 11, color: labelColor)),
                ],
              ),
            ...
            Container(
              ...
              child: Text(statusText, style: TextStyle(fontSize: 10, color: labelColor, fontWeight: FontWeight.w600)),
            ),
          ],
        ),
      ),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Table ${order.tableNo} — Order ${order.orderNo}',
                          style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF1A2035))),
                      Text('${order.guests} guests · ${order.minutesAgo} min ago',
          ...
          Padding(
            ...
            child: Wrap(
              ...
              children: order.items.map((item) => Container(
                ...
                child: Text(item, style: const TextStyle(fontSize: 12, color: Color(0xFF1A2035))),
              )).toList(),
            ),
          ),
            ...
            child: Row(
              children: [
                if (order.status == OrderStatus.waiting)
                  _ActionBtn(icon: Icons.check_rounded, label: 'Mark Served', primary: true, onTap: onMarkServed),
                if (order.status == OrderStatus.billRequested)
                  _ActionBtn(icon: Icons.send_rounded, label: 'Send to Cashier', primary: true, onTap: onSendToCashier),
                if (order.status == OrderStatus.served) ...[
                  _ActionBtn(icon: Icons.add_rounded, label: 'Add Items', primary: false, onTap: onAddItems),
                  const SizedBox(width: 8),
                  _ActionBtn(icon: Icons.receipt_long_rounded, label: 'Request Bill', primary: false, onTap: onRequestBill),
                ],
                if (order.status != OrderStatus.served) ...[
                  const SizedBox(width: 8),
                  _ActionBtn(icon: Icons.edit_rounded, label: 'Edit Order', primary: false, onTap: () {}),
                  const SizedBox(width: 8),
                  _ActionBtn(icon: Icons.chat_bubble_outline_rounded, label: 'Note', primary: false, onTap: () {}),
                ],
              ],
            ),
          ),
...
class _ActionBtn extends StatelessWidget {
  ...
  final bool primary;
  final VoidCallback onTap;
  const _ActionBtn({required this.icon, required this.label, required this.primary, required this.onTap});
...
class _OrderData {
  ...
  const _OrderData({required this.tableNo, required this.orderNo, required this.guests, required this.minutesAgo, required this.status, required this.items});
}
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                Container(width: 36, height: 36, decoration: BoxDecoration(color: _amber, borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.restaurant_rounded, color: Colors.white, size: 20)),
