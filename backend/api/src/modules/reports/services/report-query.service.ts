...
export class ReportQueryService {
  ...
  async salesSummary(filter: ReportFilterDto, user: AuthenticatedUser) {
    return this.run('SALES_SUMMARY', filter, user, GENERAL_REPORT_ROLES, async (tx, context) => {
      ...
      const discounts = sum(
        bills,
        (bill) => bill.discountAmount + bill.couponDiscountAmount,
      );
  ...
  async salesTrend(
    ...
    return this.run(`SALES_${period}`, filter, user, GENERAL_REPORT_ROLES, async (tx, context) => {
      ...
      for (const bill of bills) {
        ...
        const key = period === 'YEAR' ? iso.slice(0, 4) : period === 'MONTH' ? iso.slice(0, 7) : iso.slice(0, 10);
  ...
  async itemSales(filter: ReportFilterDto, user: AuthenticatedUser) {
    return this.run('ITEM_SALES', filter, user, GENERAL_REPORT_ROLES, async (tx, context) => {
      ...
      const groups = new Map<string, { name: string; category: string; quantity: number; revenue: number; cost: number }>();
  ...
  async categorySales(filter: ReportFilterDto, user: AuthenticatedUser) {
    ...
    for (const item of items) {
      const current = groups.get(item.category) ?? { sales: 0, quantity: 0, items: new Set<string>() };
  ...
  async gst(filter: ReportFilterDto, user: AuthenticatedUser, detailed: boolean) {
    return this.run(detailed ? 'GST_DETAILED' : 'GST_SUMMARY', filter, user, GENERAL_REPORT_ROLES, async (tx, context) => {
      const bills = await tx.bill.findMany({
        where: this.billWhere(context),
        select: {
          id: true,
  ...
  async payments(filter: ReportFilterDto, user: AuthenticatedUser, mode: 'SUMMARY' | 'METHODS' | 'REFUNDS') {
    return this.run(`PAYMENTS_${mode}`, filter, user, GENERAL_REPORT_ROLES, async (tx, context) => {
      const payments = await tx.payment.findMany({
        where: {
          ...
          ...(filter.paymentMethod ? { transactions: { some: { paymentMethod: filter.paymentMethod } } } : {}),
        },
      ...
      for (const payment of payments.filter((item) => item.status !== PaymentStatus.FAILED)) {
        for (const transaction of payment.transactions) {
          methods[transaction.paymentMethod] = (methods[transaction.paymentMethod] ?? 0) + transaction.amount;
        }
      }
      ...
      return mode === 'METHODS'
        ...
        : {
            ...
            refunds,
            failedPayments,
            totalCollected: Object.values(methods).reduce((total, amount) => total + amount, 0) - refunds,
          };
    });
  }

  async inventory(filter: ReportFilterDto, user: AuthenticatedUser, mode: 'VALUE' | 'CONSUMPTION' | 'WASTAGE' | 'LOW_STOCK') {
    return this.run(`INVENTORY_${mode}`, filter, user, INVENTORY_REPORT_ROLES, async (tx, context) => {
      if (mode === 'VALUE') {
  ...
  async customers(filter: ReportFilterDto, user: AuthenticatedUser, mode: 'TOP' | 'NEW' | 'REPEAT') {
    return this.run(`CUSTOMERS_${mode}`, filter, user, MANAGEMENT_REPORT_ROLES, async (tx, context) => {
      ...
      const groups = new Map<string, { customer: string; orders: Set<string>; spend: number; visits: number }>();
  ...
  async outletPerformance(filter: ReportFilterDto, user: AuthenticatedUser) {
    return this.run('OUTLET_PERFORMANCE', filter, user, MANAGEMENT_REPORT_ROLES, async (tx, context) => {
      ...
      for (const outlet of outlets) {
        const [bills, visits, stocks] = await Promise.all([
          tx.bill.findMany({
            ...
            select: { orderId: true, paidAmount: true, refundedAmount: true, items: { select: { quantity: true, orderItem: { select: { menuItem: { select: { costPrice: true } } } } } } },
          }),
          tx.customerVisit.count({ where: { outletId: outlet.id, businessDate: dateRange(context) } }),
          tx.inventoryStock.findMany({ where: { outletId: outlet.id }, include: { ingredient: { select: { costPrice: true } } } }),
        ]);
        const sales = sum(bills, (bill) => bill.paidAmount - bill.refundedAmount);
        const cost = bills.reduce((total, bill) => total + bill.items.reduce((subtotal, item) => subtotal + item.quantity * (item.orderItem.menuItem.costPrice ?? 0), 0), 0);
        results.push({
          outletId: outlet.id,
          outlet: outlet.name,
          sales,
          ...
          customers: visits,
          profit: sales - cost,
          inventoryValue: Math.round(stocks.reduce((total, stock) => total + Number(stock.availableQuantity) * stock.ingredient.costPrice, 0)),
        });
      }
      return results;
    });
  ...
  async kitchen(filter: ReportFilterDto, user: AuthenticatedUser) {
    return this.run('KITCHEN_PERFORMANCE', filter, user, KITCHEN_REPORT_ROLES, async (tx, context) => {
      ...
      const averagePrepTime = prepared.length
        ? Number((prepared.reduce((total, item) => total + (item.actualPrepMinutes ?? 0), 0) / prepared.length).toFixed(2))
        : 0;
      return {
  ...
  async staff(filter: ReportFilterDto, user: AuthenticatedUser) {
    ...
    return this.run('STAFF_PERFORMANCE', { ...filter, employeeId }, user, STAFF_REPORT_ROLES, async (tx, context) => {
      ...
      const payments = await tx.payment.findMany({
        where: {
          ...
          status: { in: [PaymentStatus.SUCCESS, PaymentStatus.PARTIALLY_PAID, PaymentStatus.REFUNDED] },
        },
        select: { createdByUserId: true, paidAmount: true, refundedAmount: true },
      });
      const ids = [...new Set([...orders.map((row) => row.waiterId), ...payments.map((row) => row.createdByUserId)].filter((id): id is string => id !== null))];
      const users = await tx.userAccount.findMany({ where: { id: { in: ids } }, select: { id: true, displayName: true } });
      ...
      return ids.map((id) => {
        ...
        return {
          ...
          waiterSales,
          cashierCollections: sum(payments.filter((payment) => payment.createdByUserId === id), (payment) => payment.paidAmount - payment.refundedAmount),
          ordersHandled: employeeOrders.length,
          averageTicketSize: employeeOrders.length ? Math.round(waiterSales / employeeOrders.length) : 0,
        };
      });
    });
  }
    ...
    return this.run('PLATFORM_SUMMARY', filter, user, ['SUPER_ADMIN'], async (tx, context) => {
      ...
      return {
        ...
        topTenants: [...grouped.entries()]
          .map(([tenantId, row]) => ({ tenantId, tenant: tenantNames.get(tenantId), revenue: row.revenue, orders: row.orders.size }))
  ...
  async dashboard(filter: ReportFilterDto, user: AuthenticatedUser) {
    return this.run('DASHBOARD', filter, user, GENERAL_REPORT_ROLES, async (tx, context) => {
      const [bills, visits, pendingOrders, stocks, lowStockAlerts, billItems] =
        await Promise.all([
          tx.bill.findMany({
            where: this.billWhere(context),
            select: {
  ...
  private async run<T>(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.reportGenerationAudit.create({
...
function taxAmount(
  taxes: Array<{ taxName: string; taxAmount: number }>,
  name: string,
): number {
  return taxes
