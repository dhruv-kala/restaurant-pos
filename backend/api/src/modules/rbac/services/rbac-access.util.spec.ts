...
describe('RBAC access rules', () => {
  it.each(['SUPER_ADMIN', 'TENANT_ADMIN', 'MANAGER'])(
    'allows %s to read RBAC data',
    (role) => {
      expect(() => requireRbacRead(user([role]))).not.toThrow();
    },
  );

  it.each(['SUPER_ADMIN', 'TENANT_ADMIN'])(
    'allows %s to mutate RBAC data',
    (role) => {
  ...
  it('denies operational roles and manager writes', () => {
    expect(() => requireRbacRead(user(['CASHIER']))).toThrow(
      ForbiddenException,
    );
    expect(() => requireRbacWrite(user(['MANAGER']))).toThrow(
      ForbiddenException,
    );
  });
  ...
  it('requires explicit tenant scope for platform administration', () => {
    expect(() =>
      resolveRbacTenantId(undefined, user(['SUPER_ADMIN'], null, null)),
    ).toThrow(BadRequestException);
  });

  it('requires manager outlet context', () => {
    expect(managerOutletId(user(['MANAGER']))).toBe(
      '00000000-0000-4000-8000-000000000002',
    );
    expect(() => managerOutletId(user(['MANAGER'], undefined, null))).toThrow(
      ForbiddenException,
    );
  });
});
