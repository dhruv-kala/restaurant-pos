...
The root `packages/` directory contains explicit Dart/Flutter workspace packages.
Application-specific widgets remain inside their owning app.
...
|---|---|---|
| `mobile-app/restaurantpos` | `apps/restaurant-app` | Identify the Flutter client as a deployable application |
| `webapp` | `apps/admin` | Place the future admin client with other applications |
| `api` | `backend/api` | Establish backend ownership and future module boundaries |
| Root `shared` placeholder | `packages` | Establish explicit reusable package ownership |

### Flutter application

| Previous path | New path | Reason |
|---|---|---|
| `lib/screens/admin/admin_screen.dart` | `lib/features/dashboard/presentation/screens/outlet_dashboard_screen.dart` | Outlet-manager dashboard ownership and consistent naming |
| `lib/screens/cashier/cashier_screen.dart` | `lib/features/pos/presentation/screens/cashier_screen.dart` | Feature ownership |
| `lib/screens/waiter/waiter_screen.dart` | `lib/features/tables/presentation/screens/waiter_screen.dart` | Table-service feature ownership |
...
| Authenticated user/role contract | `packages/auth` | Shared identity semantics without Firebase coupling |
| Three private sidebar copies | `packages/ui_kit/lib/src/navigation/app_sidebar.dart` | Remove duplicated application chrome |
| Three private nav item models | `packages/ui_kit/lib/src/navigation/app_navigation_item.dart` | One navigation contract |
| Cashier-local menu model | `packages/shared_models/lib/src/menu/menu_item.dart` | Share a stable menu contract |
...
| App theme | `packages/ui_kit/lib/src/theme/app_theme.dart` | Reusable visual foundation |
| Errors and currency formatter | `packages/core/lib/src` | Framework-independent shared utilities |
...
```text
apps/restaurant-app/lib/core/database/
apps/restaurant-app/lib/core/sync/
...
5. **Checkpoint E - Monorepo vocabulary:** application names aligned and
   documentation grouped by ownership.
6. **Checkpoint F - Workspace packages:** core, auth, shared models, and UI kit
   extracted behind public package APIs.
7. **Checkpoint G - Validation:** root workspace resolution, format, analyzer,
...
| Risk | Mitigation |
|---|---|
| Tooling assumes old Flutter path | Update CI/IDE commands to `apps/restaurant-app` |
...
| Future apps import restaurant-app internals | Workspace package ownership and dependency linting |
...
3. Move `apps/restaurant-app` back to `mobile-app/restaurantpos`,
   `apps/admin` back to `webapp`, and `backend/api` back to `api`.
